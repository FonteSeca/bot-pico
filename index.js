...
oie.kick();
  ...
  if ((/!novatu/).test(message.content)) {
    ...
    if (message.mentions.users.size > 0) {
      if(message.member.roles.find("name", "Moderador") || message.member.roles.find("name", "Estagiário") || message.member.roles.find("name", "☕Seitokai")) {
        const member = msg.guild.member(message.mentions.users.first());
  ...
  if ((/!luar/).test(message.content)) {
    const mention = message.mentions.users.first();
    message.channel.send(Bot.user.username + ', convidou ' + mention.toString() +' para a gozada lunática', {file: "https://cdn.discordapp.com/attachments/302911279608233986/358439411798638592/I_5dd976ad9e94cfc9f63f249955a697bb.gif"}); // Mensagem normal
    message.delete()
    .then(msg => console.log(`Deleted message from ${msg.author}`))
    .catch(console.error);
  }
